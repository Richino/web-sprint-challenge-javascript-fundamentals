...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
//Because the nested function and the internal variable are in the same scope
...
function lowerCaseNames(...array) {
	array = zooAnimals.map((element) => {
		return element.animal_name.toLocaleLowerCase();
	});
...
function lowPopulationAnimals(...array) {
	array = zooAnimals.filter((element) => {
...
function USApop(totalPopulation) {
	let getPopulation = zooAnimals.map((element) => {
		return element.population;
	});

	totalPopulation = getPopulation.reduce((num, element) => {
		return num + element;
	}, 0);
